...
// bool AMCLLaser::UpdateSensor(pf_t *pf, AMCLSensorData *data, rk_loc::LandmarkLoc landmark_localisator)
// {
//   if (this->max_beams < 2)
//     return false;
//
...
//     pf_update_sensor(pf, (pf_sensor_model_fn_t) BeamModel, data, landmark_localisator);
//   else if(this->model_type == LASER_MODEL_LIKELIHOOD_FIELD)
//     pf_update_sensor(pf, (pf_sensor_model_fn_t) LikelihoodFieldModel, data, landmark_localisator);
//   else if(this->model_type == LASER_MODEL_LIKELIHOOD_FIELD_PROB)
//     pf_update_sensor(pf, (pf_sensor_model_fn_t) LikelihoodFieldModelProb, data, landmark_localisator);
//   else
//     pf_update_sensor(pf, (pf_sensor_model_fn_t) BeamModel, data, landmark_localisator);
//
//   return true;
...
bool AMCLLaser::UpdateSensor(pf_t *pf, AMCLSensorData *data, geometry_msgs::PoseWithCovarianceStamped landmark_loc_pose)
{
  ...
  // Apply the laser sensor model
  if(this->model_type == LASER_MODEL_BEAM)
    pf_update_sensor(pf, (pf_sensor_model_fn_t) BeamModel, data, current_pose, covariances);
  else if(this->model_type == LASER_MODEL_LIKELIHOOD_FIELD)
    pf_update_sensor(pf, (pf_sensor_model_fn_t) LikelihoodFieldModel, data, current_pose, covariances);
  else if(this->model_type == LASER_MODEL_LIKELIHOOD_FIELD_PROB)
    pf_update_sensor(pf, (pf_sensor_model_fn_t) LikelihoodFieldModelProb, data, current_pose, covariances);
  else
    pf_update_sensor(pf, (pf_sensor_model_fn_t) BeamModel, data, current_pose, covariances);
